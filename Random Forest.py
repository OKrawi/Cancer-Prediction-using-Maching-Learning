...
print("Accuracy of RF algorithm: {:.3f} %".format(accuracy_score(y_test, y_pred)))
print("F1 Score of RF algorithm: {:.3f} %".format(f1_score(y_test, y_pred)))
print("Recall Score of RF algorithm: {:.3f} %".format(recall_score(y_test, y_pred)))
print("Precision Score of RF algorithm: {:.3f} %".format(precision_score(y_test, y_pred)))

# Getting the Cross Validation score
cv = RepeatedKFold(n_splits=10, n_repeats=1, random_state=1)
scores = cross_val_score(rf_classifier, X, y, scoring='accuracy', cv=cv, n_jobs=-1)
print(f'Cross Validation Accuracy of RF algorithm: {np.mean(scores):.3f} ({np.std(scores):.3f})')

# Getting the AUC score
pred_prob = rf_classifier.predict_proba(X_test)
auc_score = roc_auc_score(y_test, pred_prob[:, 1])
print("AUC Score of RF algorithm: {:.3f} %".format(auc_score))
...
ax.set_title('Confusion Matrix for RF Algorithm\n\n')
ax.set_xlabel('\nPredicted Values')
