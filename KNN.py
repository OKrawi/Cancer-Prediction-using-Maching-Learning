...
# Creating the KNN classifier and fitting the data
...
print("Accuracy of KNN algorithm: {:.3f} %".format(accuracy_score(y_test, y_pred)))
print("F1 Score of KNN algorithm: {:.3f} %".format(f1_score(y_test, y_pred)))
print("Recall Score KNN DT algorithm: {:.3f} %".format(recall_score(y_test, y_pred)))
print("Precision Score KNN DT algorithm: {:.3f} %".format(precision_score(y_test, y_pred)))
...
print(f'Cross Validation Accuracy of KNN algorithm: {np.mean(scores):.3f} ({np.std(scores):.3f})')

# Getting the AUC score
pred_prob = knn_classifier.predict_proba(X_test)
auc_score = roc_auc_score(y_test, pred_prob[:, 1])
print("AUC Score of KNN algorithm: {:.3f} %".format(auc_score))
